...
int get_variable_flag_c(const char* base_url, const char* session_id, int var_id) {
    CURL *curl;
    CURLcode res;
    char url[512];
    int flag_status = -1;  // Default to -1 to indicate an error

    snprintf(url, sizeof(url), "%s/get_variable_flag?session_id=%s&var_id=%d", base_url, session_id, var_id);
